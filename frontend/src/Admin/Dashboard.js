...
useEffect(() => {
  const fetchAdmin = async () => {
    try {
      console.log('adminId:', adminId); // Check the value of adminId

      if (!adminId) {
        return; // Exit early if adminId is not defined
      }
  ...
  fetchAdmin(); // Always call fetchAdmin, regardless of adminId
}, [adminId]);
    ...
    Modal.confirm({
      ...
      onOk: () => {
        ...
        form.validateFields().then((values) => {
          ...
          // Update admin data
          axios
            .put(`http://localhost:3000/Users/${adminId}`, updatedAdmin)
            .then((response) => {
              if (response.status === 200) {
                // Upload file separately
                axios
                  .put(`http://localhost:3000/Users/${adminId}`, formData)
                  .then((uploadResponse) => {
                    if (uploadResponse.status === 200) {
                      message.success('Admin data and file updated successfully.');
